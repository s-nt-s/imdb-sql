...
def _to_tuple(s: str, sep: str):
    arr: list[str] = []
    for i in map(str.strip, re.split(r"\s*"+re.escape(sep)+r"\s*", s or '')):
        if len(i) and i not in arr:
            arr.append(i)
    return tuple(arr)
...
def _parse_key_val(k: str, v: Any):
    ...
    if v is None:
        ...
        if k in ('directors', 'writers'):
            return tuple()
        return None
    if k in ('isOriginalTitle', 'isAdult'):
        if v not in ('0', '1', 1, 0, True, False):
            raise KeyValueError(f"{k} = {v}")
        return int(v) == 1
    if not isinstance(v, str):
        return v
    if k in ('directors', 'writers', 'genres'):
        return _to_tuple(v, sep=',')
